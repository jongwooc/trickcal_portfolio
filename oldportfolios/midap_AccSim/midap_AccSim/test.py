...
def parse():
    ...
    parser.add_argument('-n', '--network', type=str, choices=list(
        custom_examples.keys()) + ["all", "test"], required=True)
    ...
    parser.add_argument('-da', '--disable_abstract_layer',
                        action="store_true", default=False)
    parser.add_argument('-dr', '--disable_reduction_layer',
                        action="store_true", default=False)
    parser.add_argument('-f', '--fmem_entries', type=int, default=256)
    parser.add_argument('-w', '--wmem_entries', type=int, default=32)
    ...
    parser.add_argument('--load', type=str,
                        choices=['ONE', 'MAXIMUM'], default='ONE')
    parser.add_argument('-or', '--output_enable_read_stats',
                        action="store_true", default=False)
    parser.add_argument('-ow', '--output_enable_write_stats',
                        action="store_true", default=False)
    parser.add_argument('-oc', '--output_enable_components_stats',
                        action="store_true", default=False)
    parser.add_argument('-oe', '--output_enable_etc_stats',
                        action="store_true", default=False)
    parser.add_argument('-oa', '--output_enable_all_stats',
                        action="store_true", default=False)
...
custom_examples = {
    ...
    #added by Jongwoo
    'squeezenet': (lambda x: squeezenet.squeezenet11(input_size=x[-1]) if x is not None else squeezenet.squeezenet11())
}

test_set = ['resnet50', 'wide_resnet50_2',
            'se_resnet50', 'mobilenet_v2', 'inception_v3']
...
input_shape = None if args.input_size == 0 else (
...
if args.network == 'all':
    ...
    for network in test_set:
        ...
        latency, dram_access = tr.run_all(
            mb, output_dir=output_dir, output_option=output_option)
        network_id = network
        if args.layer_compiler == 'MIN_DRAM_ACCESS':
            network_id += '_m'
        expected_latency, expected_dram_access = expected_stats[network_id]
        lat_diff = abs(expected_latency - latency)
        access_diff = abs(expected_dram_access - dram_access)
        if latency == 0 or dram_access == 0:
            logger.error("Impossible result. (Latency, Dram access) = {}".format(
                (latency, dram_access)))
        lat_diff_rate = lat_diff / expected_latency
        access_diff_rate = access_diff / expected_dram_access
        if lat_diff_rate > 0.05:
            logger.warning("Is something changed? simulated latency for network [ {} ] has changed too much ( {} )".format(
                network, lat_diff_rate))
        if lat_diff_rate > 0.05:
            logger.warning("Is something changed? simulated dram access for network [ {} ] has changed too much ( {} )".format(
                network, access_diff_rate))

elif args.network in custom_examples:
    if input_shape is None:
        raise ValueError(
            "you must specify image_size w/ --image_size or -i $IMG_SIZE to test a specific model")
    mb = custom_examples[args.network](input_shape)
    tr.run_all(mb, output_dir=output_dir, output_option=output_option)

else:
    raise ValueError(
        "{} is not supported model.. please check custom_examples list.".format(args.network))
