...
from generic_op import ArithmeticOp, ConcatOp, ConvOp, PoolOp
...
class DRAM(object):
    def __init__(self, layer):
        ...
        self.dram_read_latency = cfg.LATENCY.DRAM_READ
    ...
    def _set_unit_time(self, layer):
        ...
        last_time = div_ceil(
            last_input_size, self.bandwidth) if last_input_size != 0 else typical_time
    ...
    def request_fmem(self, current_time, fragments, is_last=False):
        ...
        for f in fragments:
            data_cycle = self._calc_fmem_load_cycle(
                1, (is_last if f == fragments[-1] else False))
    ...
    def get_dram_latency(self, size):
        return self.dram_read_latency * ceil(size / self.dram_page_size)
    ...
    def get_wmem_delay_time(self, current_time, filter_num):
        layer = self.layer
        if isinstance(layer.main_op, ConcatOp) or isinstance(layer.main_op, ArithmeticOp) or isinstance(layer.main_op, PoolOp):
            return 0
...
class CIM(object):
    ...
    def _set_unit_time(self, layer):
        ...
        # 0: whole bank 1: partial bank
        self.unit_time = [typical_time /
                          self.filter_num, last_time / self.filter_num]
...
class TimeLine(object):
    def __init__(self, layer, prev):
        ...
        if isinstance(layer.main_op, ConcatOp) or isinstance(layer.main_op, ArithmeticOp):
            self.cim = None
            self.dram = None
            return
        ...
        if isinstance(layer.main_op, ConvOp) and 'input' not in layer.input[0].name:
    ...
    def limit_processing_fragments(self, fragments, bound, max_frag_num, is_last):
        ...
        if isinstance(self.layer.main_op, ArithmeticOp):
            return max_frag_num
        if isinstance(self.layer.main_op, PoolOp):
            return 1
        # XXX
        if self.layer.main_op.type == 'Depthwise':
            max_frag_num = 1
