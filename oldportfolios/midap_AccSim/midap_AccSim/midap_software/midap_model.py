...
class MidapModel(OrderedDict):
    ...
    def from_generic_op_dict(self, op_dict):
        ...
        for op_key in op_dict:
            ...
            elif isinstance(op, UpsampleOp) and allow_abstract:
            ...
            # or op.type in ['Add', 'Sum', 'Mul']:
            elif isinstance(op, PoolOp) or isinstance(op, UpsampleOp):
                if finish:
                    raise ModelBuildError("post_processing already started")
                ...
                finish = True
                print(
                    '{}: Not supported operator type < {} > considered as a post-processing step begins'.format(op.name, op.type))
        self._post_process()
    ...
    def _add_layer(self, op):
        ...
        if mapping_op is not None:
            if len(op.input_layers) > 1:
                raise ValueError(
                    "v1.3.0) Multi-input layers with mapping func are not supported yet")
                # How to support it?
            else:
        ...
        self.last_modified = layer.name

    def _update_layer(self, op):
        if self.last_modified not in op.input_layers:
            self._add_layer(op)
        if self[self.last_modified].set_sub_op(op):
            layer = self[self.last_modified]
            self[layer.name] = layer
            del self[self.last_modified]
            self.last_modified = layer.name
        else:
    ...
    def _post_process(self):
        ...
        for key in self:
            if key in self.init_layer:
                continue
            layer = self[key]
            if len(layer.next) == 0 or \
                    (len(layer.next) == 1 and layer.have_reduction_layer):
            ...
            if isinstance(layer.main_op, ArithmeticOp):
                inl = layer.input
                # TODO duseok ,for refactoring
                if inl[0].output_tensor.size != inl[1].output_tensor.size:
                    layer.main_op.broadcast = True
                    if not (inl[0].output_tensor[:, :, 0].size == 1 or inl[1].output_tensor[:, :, 0].size == 1):
    ...
    def _make_graph(self):
        for key in self:
            ...
            # make graph
            if layer.reduction:
                pad = self[layer.input[0]].output_tensor.shape[-1] - layer.output_tensor.shape[-1]
                if pad > 0:
    ...
    def _setup_pad_feature_map(self, layer):
        if layer.main_op.type == 'Conv' and layer.mapping_func is None and layer.dilation == 1:
    ...
    def _setup_concat(self, layer):
        ...
        for concat_layer, offset in (zip(layer.input, layer.main_op.size_info)):
            if len(self[concat_layer].next) == 1:
                self[concat_layer].offset = (offset, offset + self[concat_layer].offset[1])
                # self[concat_layer].offset = (offset, prev)
                # prev = offset
