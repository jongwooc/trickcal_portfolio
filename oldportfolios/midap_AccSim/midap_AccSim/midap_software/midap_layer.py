...
class MidapLayer(object):
    def __init__(self, main_op, mapping_op=None, sub_op=None, reduction=False):
        ...
        if isinstance(mapping_op, UpsampleOp):
            ...
            if mapping_op.algorithm == 'Zero':
                self.valid_func = lambda x, y: (
                    x % scale_w) == 0 and (y % scale_h) == 0
            else:
        ...
        self.processed_input = 0  # for control info generation
    ...
    def set_sub_op(self, sub_op):
        ...
        if self.sub_op is None and any([isinstance(self.main_op, ConvOp) and isinstance(sub_op, PoolOp), isinstance(sub_op, UpsampleOp)]):
            self.sub_op = sub_op
    ...
    def setup(self):
        ...
        # if isinstance(self.main_op, ConvOp):
        #     self._num_planes_per_wmem = cfg.MIDAP.WMEM.NUM_ENTRIES // np.prod(self.main_op.weight.shape[1:])
        #     self.require_wmem = div_ceil(self.main_op.weight.shape[1], self._num_planes_per_wmem * cfg.MIDAP.WMEM.NUM)
        # else:
        #     self.require_wmem = 0
        ...
        if isinstance(self.main_op, ConvOp) and input_layer and input_layer.num_planes_per_fmem < self.main_op.k_w - 1:
    ...
    def get_output_fragments(self, num_bank):
        control_info = self.control_info
        output_fragments = self.get_fragments(num_bank, 0)
    ...
    def get_fragments(
        ...
        overlap_load = overlap and isinstance(
            main_op, ConvPoolOpBase) and main_op.type != 'Gemm'
        fixed_plane_size = self.num_planes_per_fmem
        if overlap_load and main_op.k_w > 1:
            head = layer.scale_w * head
            head = head - (main_op.k_w - 1) * layer.dilation
            head = div_ceil(head - main_op.pad_l, main_op.stride) * \
                main_op.stride + main_op.pad_l
            if layer.mapping_func is not None:
                head, _ = layer.mapping_func(head, 0)
        fragments = []
        for i in range(num_fragments):
            if head >= input_shape[0]:
                break
            tail = min(head + fixed_plane_size, input_shape[0])
    ...
    def get_output_x(self, fragment, next_available=False):
        ...
        input_x = self.scale_w * fragment[1] - 1
        main_x = input_x
        main_op = self.main_op
        if input_x >= self.scale_w * input_shape[0] - 1:
    ...
    def num_planes_per_fmem(self):
        return (self._num_planes_per_fmem if self.main_op.type == 'HEAD' or self.main_op.output_tensor.shape[0] >= self._num_planes_per_fmem else self.output_tensor.shape[0])
    ...
    def get_input_shape(self):  # This func might be affected
        ...
        chan = reduce(lambda x, y: x +
                      y.main_op.output_tensor.shape[2], [0] + self.input)
        return shape + [chan]
