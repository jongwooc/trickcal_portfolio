...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < caffe_model->layer_size(); i ++)  {
        ...
        if (caffe_model->layer(i).name() == time->layer(temp_i).name()) {
            ...
            if (time->layer(temp_i).has_gpu())   {
                gpu_time.push_back(0);
            }
            if (time->layer(temp_i).has_dsp())    {
                dsp_time.push_back(0);
            }
            if (time->layer(temp_i).has_npu())   {
                npu_time.push_back(0);
            }
            if (time->layer(temp_i).has_cpu2())    {
                cpu2_time.push_back(0);
            }
            if (time->layer(temp_i).has_cpu3())    {
                cpu3_time.push_back(0);
            }
            if (time->layer(temp_i).has_cpu4())    {
                cpu4_time.push_back(0);
            }
        }
    ...
    if (caffe_model->layer_size() != layer.size()){
    ...
    try {
        ...
        GRBVar CPU_sum = model.addVar(0.0, 999999999.0, 0.0, GRB_CONTINUOUS, "CPU sum");
        GRBVar GPU_sum = model.addVar(0.0, 999999999.0, 0.0, GRB_CONTINUOUS, "GPU sum");
        GRBVar CPU2_sum = model.addVar(0.0, 999999999.0, 0.0, GRB_CONTINUOUS, "CPU2 sum");
        GRBVar CPU3_sum = model.addVar(0.0, 999999999.0, 0.0, GRB_CONTINUOUS, "CPU3 sum");
        GRBVar CPU4_sum = model.addVar(0.0, 999999999.0, 0.0, GRB_CONTINUOUS, "CPU4 sum");
        ...
        if (layer_cpu3.size())  {
            ...
            for (int i = 0; i < layer.size(); i ++)   {
                for (int j = 0; j < i; j ++)   {
                    if (layer_comm[j][i]) {
                        std::string temp_string0 = layer[i];
                        temp_string0 = temp_string0 + "to_CPU3";
                        GRBLinExpr addConstrlhs0 = 0;
                        for (int k = 0; k < bottom_layer[i].size(); k ++)   {
                            addConstrlhs0 += (1-layer_cpu3[j+k]);
                        }
            ...
            for (int i = 0; i < layer.size(); i ++)   {
                for (int j = 0; j < i; j ++)   {
                    if (layer_comm[j][i]) {
                        for (int k = 0; k < bottom_layer[i].size(); k ++)   {
                            std::string temp_string0 = layer[i];
                            temp_string0 = temp_string0 + "_to_CPU3_"+layer[j+k];
                            GRBLinExpr addConstrlhs0 = 0;
                            addConstrlhs0 += (1-layer_cpu3[j+k]);
                            model.addConstr(addConstrlhs0,'<',to_CPU3_transition[i],temp_string0);
                        }
        ...
        if (layer_cpu4.size())  {
            ...
            for (int i = 0; i < layer.size(); i ++)   {
                for (int j = 0; j < i; j ++)   {
                    if (layer_comm[j][i]) {
                        std::string temp_string0 = layer[i];
                        temp_string0 = temp_string0 + "to_CPU4";
                        GRBLinExpr addConstrlhs0 = 0;
                        for (int k = 0; k < bottom_layer[i].size(); k ++)   {
                            addConstrlhs0 += (1-layer_cpu4[j+k]);
                        }
            ...
            for (int i = 0; i < layer.size(); i ++)   {
                for (int j = 0; j < i; j ++)   {
                    if (layer_comm[j][i]) {
                        for (int k = 0; k < bottom_layer[i].size(); k ++)   {
                            std::string temp_string0 = layer[i];
                            temp_string0 = temp_string0 + "_to_CPU4_"+layer[j+k];
                            GRBLinExpr addConstrlhs0 = 0;
                            addConstrlhs0 += (1-layer_cpu4[j+k]);
                            model.addConstr(addConstrlhs0,'<',to_CPU4_transition[i],temp_string0);
                        }
        ...
        for (int i = 0; i < layer.size(); i ++)   {
            ...
            GRBLinExpr addConstrlhs0 = 0;
            addConstrlhs0 += to_CPU_comm[i]  - pre_cpu[i];
            if (layer_cpu2.size())  {
                addConstrlhs0 += to_CPU2_comm[i] - pre_cpu2[i];
            }
            if (layer_cpu3.size())  {
                addConstrlhs0 += to_CPU3_comm[i] - pre_cpu3[i];
            }
            if (layer_cpu4.size())  {
                addConstrlhs0 += to_CPU4_comm[i] - pre_cpu4[i];
            }
        ...
        for (int i = 0; i < layer.size(); i ++)   {
            for (int j = 0; j < i; j ++)   {
                if (layer_comm[j][i]) {
                    ...
                    GRBLinExpr addConstrlhs = 0;
                    addConstrlhs += layer_start[i] - layer_end[j] - (map[i] * (map_alpha * layer_comm[j][i] + map_beta) +  unmap[i] * (unmap_alpha * layer_comm[j][i] + unmap_beta));
...
//            float unmap_beta = 526.39;
            ...
            model.addConstr(tempConstrlhs,'=',CPU_sum, "CPU sum");
            ...
            model.addConstr(tempConstrlhs,'=',GPU_sum, "GPU sum");
            ...
            if (layer_cpu2.size())  {
                ...
                model.addConstr(tempConstrlhs,'=',CPU2_sum, "CPU2 sum");
            }
            if (layer_cpu3.size())  {
                tempConstrlhs = 0;
                for (int i = 0; i < layer.size(); i ++)   {
                ...
                model.addConstr(tempConstrlhs,'=',CPU3_sum, "CPU3 sum");
            }
            if (layer_cpu4.size())  {
                tempConstrlhs = 0;
                for (int i = 0; i < layer.size(); i ++)   {
                ...
                model.addConstr(tempConstrlhs,'=',CPU4_sum, "CPU4 sum");
            }
            
            tempConstrlhs = 0;
            tempConstrlhs += CPU_sum;
            model.addConstr(tempConstrlhs,'<',MAX_ob,"bigger than CPU_sum");
            tempConstrlhs = 0;
            tempConstrlhs += GPU_sum;
            model.addConstr(tempConstrlhs,'<',MAX_ob,"bigger than GPU_sum");
            tempConstrlhs = 0;
            tempConstrlhs += CPU_sum;
            model.addConstr(tempConstrlhs,'>',min_ob,"smaller than CPU_sum");
            tempConstrlhs = 0;
            tempConstrlhs += GPU_sum;
            model.addConstr(tempConstrlhs,'>',min_ob,"smaller than GPU_sum");
            
            
            if (layer_cpu2.size())  {
                tempConstrlhs = 0;
                tempConstrlhs += CPU2_sum;
                model.addConstr(tempConstrlhs,'<',MAX_ob,"bigger than CPU2_sum");
                tempConstrlhs = 0;
                tempConstrlhs += CPU2_sum;
                model.addConstr(tempConstrlhs,'>',min_ob,"smaller than CPU2_sum");
            }
            if (layer_cpu3.size())  {
                tempConstrlhs = 0;
                tempConstrlhs += CPU3_sum;
                model.addConstr(tempConstrlhs,'<',MAX_ob,"bigger than CPU3_sum");
                tempConstrlhs = 0;
                tempConstrlhs += CPU3_sum;
                model.addConstr(tempConstrlhs,'>',min_ob,"smaller than CPU3_sum");
            }
            if (layer_cpu4.size())  {
                tempConstrlhs = 0;
                tempConstrlhs += CPU4_sum;
                model.addConstr(tempConstrlhs,'<',MAX_ob,"bigger than CPU4_sum");
                tempConstrlhs = 0;
                tempConstrlhs += CPU4_sum;
                model.addConstr(tempConstrlhs,'>',min_ob,"smaller than CPU4_sum");
            }
        ...
        for (int i = 0; i < layer.size(); i ++)   {
            ...
            if (layer_cpu2.size()) {
                ...
                << abs(layer_cpu2[i].get(GRB_DoubleAttr_X));
            }
            if (layer_cpu3.size()) {
                std::cout << layer_cpu3[i].get(GRB_StringAttr_VarName) <<"  "<< '\t'
                << abs(layer_cpu3[i].get(GRB_DoubleAttr_X));
            }
            if (layer_cpu4.size()) {
                std::cout << layer_cpu4[i].get(GRB_StringAttr_VarName) <<"  "<< '\t'
                << abs(layer_cpu4[i].get(GRB_DoubleAttr_X));
            }
            std::cout<<std::endl;
        }
        //layer time
...
//        }
        ...
        for (int i = 0; i < layer.size(); i ++)   {
            ...
            if (layer_cpu[i].get(GRB_DoubleAttr_X))   {
            ...
            if (layer_cpu2.size()) {
                ...
                else if (layer_cpu2[i].get(GRB_DoubleAttr_X))  {
            ...
            if (layer_cpu3.size()) {
                ...
                else if (layer_cpu3[i].get(GRB_DoubleAttr_X))  {
            ...
            if (layer_cpu4.size()) {
                ...
                else if (layer_cpu4[i].get(GRB_DoubleAttr_X))  {
