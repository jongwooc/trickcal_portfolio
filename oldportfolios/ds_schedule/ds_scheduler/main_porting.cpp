...
std::vector<int> data_for_merging;
/////////////////////////// data vector for merging!!!!!!!!!!!!!!!!!!!!!//////////////////////////////////////////////////////
...
extern "C"
{
    int ilp_5(char* model_file, char* estimation_file, float CPU_utilization, float GPU_utilization);
    int ilp(char* model_file, char* estimation_file)  {
        ilp_5(model_file,estimation_file,100, 100);
    }
    int ilp_5(char* model_file, char* estimation_file, float CPU_utilization, float GPU_utilization)  {
        int mode = 1;
        int num_of_PE = 0;
        ...
        std:: cerr << "int CPU check" << (int)CPU_utilization <<"int GPU check" << (int)CPU_utilization<<std::endl;
        //PE profile time
        ...
        // [ith layer][jth layer][0] -> map to CPU
        // [ith layer][jth layer][1] -> map to CPU2
        // [ith layer][jth layer][2] -> map to CPU3
        // [ith layer][jth layer][3] -> map to CPU4
        std::vector<std::vector<std::vector<GRBVar>>> unmap;
        
        std::vector<std::vector<GRBVar>> map_time;
        std::vector<std::vector<GRBVar>> unmap_time;
        ...
        for (int i = 0; i < caffe_model->layer_size(); i ++)  {
            if (caffe_model->layer(i).name() == time->layer(temp_i).name()) {
                ...
                if (time->layer(temp_i).has_cpu())    {
                    CPU_time.push_back(time->layer(temp_i).cpu() * (100/CPU_utilization));
                }
                if (time->layer(temp_i).has_gpu())   {
                    GPU_time.push_back(time->layer(temp_i).gpu() * (100/GPU_utilization));
                }
                ...
                if (time->layer(temp_i).has_cpu2())    {
                    CPU2_time.push_back(time->layer(temp_i).cpu2() * (100/CPU_utilization));
                }
                if (time->layer(temp_i).has_cpu3())    {
                    CPU3_time.push_back(time->layer(temp_i).cpu3() * (100/CPU_utilization));
                }
                if (time->layer(temp_i).has_cpu4())    {
                    CPU4_time.push_back(time->layer(temp_i).cpu4() * (100/CPU_utilization));
                }
                temp_i++;
...
//            for (int j = 0; j < bottom_layer[i].size(); j ++ )  {
//                std::cerr << bottom_layer[i][j]<< std::endl;
//            }
//        }
        ...
        for (int i = 0; i < layer.size(); i++){
            ...
            temp_map_set.reserve(num_of_CPU);
            
            std::vector<GRBVar> temp_unmap_set;
            temp_unmap_set.reserve(1);
            ...
            map_time.push_back(temp_map_set);
            unmap_time.push_back(temp_unmap_set);
        ...
        try {
            ...
            for (int i = 0; i < layer.size(); i ++){
                for (int j = 0; j < layer.size(); j ++){
                    std::string temp_string1 = layer[i];
                    std::string temp_string2 = layer[j];
                    std::string temp_string = temp_string1 + "_and_" + temp_string2 + "mapped";
                    GRBVar mapped = model.addVar(0.0, 1.0 ,0.0, GRB_BINARY, temp_string);
                    map[i][j].push_back(mapped);
                    temp_string = temp_string1 + "_and_" + temp_string2 + "mapped_to_CPU";
                    GRBVar mapped_to_CPU = model.addVar(0.0, 1.0 ,0.0, GRB_BINARY, temp_string);
                    map[i][j].push_back(mapped_to_CPU);
                    temp_string = temp_string1 + "_and_" + temp_string2 + "mapped_to_CPU2";
                    GRBVar mapped_to_CPU2 = model.addVar(0.0, 1.0 ,0.0, GRB_BINARY, temp_string);
                    map[i][j].push_back(mapped_to_CPU2);
                    temp_string = temp_string1 + "_and_" + temp_string2 + "mapped_to_CPU3";
                    GRBVar mapped_to_CPU3 = model.addVar(0.0, 1.0 ,0.0, GRB_BINARY, temp_string);
                    map[i][j].push_back(mapped_to_CPU3);
                    temp_string = temp_string1 + "_and_" + temp_string2 + "mapped_to_CPU4";
                    GRBVar mapped_to_CPU4 = model.addVar(0.0, 1.0 ,0.0, GRB_BINARY, temp_string);
                    map[i][j].push_back(mapped_to_CPU4);
                }
            }
            //map_time
            for (int i = 0; i < layer.size(); i ++){
                std::string temp_string = layer[i] + "CPU_map_memory_size";
                GRBVar time1 = model.addVar(0.0, 999999999.0, 0.0, GRB_CONTINUOUS, temp_string);
                map_time[i].push_back(time1);
                temp_string = layer[i] + "CPU2_map_memory_size";
                GRBVar time2 = model.addVar(0.0, 999999999.0, 0.0, GRB_CONTINUOUS, temp_string);
                map_time[i].push_back(time2);
                temp_string = layer[i] + "CPU3_map_memory_size";
                GRBVar time3 = model.addVar(0.0, 999999999.0, 0.0, GRB_CONTINUOUS, temp_string);
                map_time[i].push_back(time3);
                temp_string = layer[i] + "CPU4_map_memory_size";
                GRBVar time4 = model.addVar(0.0, 999999999.0, 0.0, GRB_CONTINUOUS, temp_string);
                map_time[i].push_back(time4);
            }
            ...
            for (int i = 0; i < layer.size(); i ++){
                for (int j = 0; j < layer.size(); j ++){
                    std::string temp_string1 = layer[i];
                    std::string temp_string2 = layer[j];
                    std::string temp_string = temp_string1 + "_and_" + temp_string2 + "unmapped";
                    GRBVar unmapped = model.addVar(0.0, 1.0 ,0.0, GRB_BINARY, temp_string);
                    unmap[i][j].push_back(unmapped);
                }
            }
            //unmap_time
            for (int i = 0; i < layer.size(); i ++){
                std::string temp_string = layer[i];
                temp_string = temp_string + "_unmap_memory_size";
                GRBVar time = model.addVar(0.0, 999999999.0, 0.0, GRB_CONTINUOUS, temp_string);
                unmap_time[i].push_back(time);
            }
            ...
            for (int i = 0; i < layer.size(); i ++)   {
                for (int j = 0; j < i; j ++)    {
                    if (layer_comm[j][i]) {
                        std::string temp_string0 = layer[i];
                        temp_string0 = temp_string0 + "to_CPU_map_1";
                        GRBLinExpr addConstrlhs0 = 0;
                        addConstrlhs0 += layer_CPU[i] + layer_GPU[j] - 1;
                        model.addConstr(addConstrlhs0,'<',map[j][i][0],temp_string0);
                    }
                }
            }
            for (int i = 0; i < layer.size(); i ++)   {
                for (int j = 0; j < i; j ++)    {
                    if (layer_comm[j][i]) {
                        std::string temp_string0 = layer[i];
                        temp_string0 = temp_string0 + "to_CPU_map_2";
                        GRBLinExpr addConstrlhs0 = 0;
                        addConstrlhs0 += layer_CPU[i];
                        model.addConstr(addConstrlhs0,'>',map[j][i][0],temp_string0);
                    }
            ...
            for (int i = 0; i < layer.size(); i ++)   {
                for (int j = 0; j < i; j ++)    {
                    if (layer_comm[j][i]) {
                        std::string temp_string0 = layer[i];
                        temp_string0 = temp_string0 + "to_CPU_map_3";
                        GRBLinExpr addConstrlhs0 = 0;
            ...
            for (int i = 0; i < layer.size(); i ++)   {
                for (int j = 0; j < i; j ++)    {
                    if (layer_comm[j][i]) {
                        std::string temp_string0 = layer[i];
                        temp_string0 = temp_string0 + "unmap_1";
                        GRBLinExpr addConstrlhs0 = 0;
                        addConstrlhs0 += layer_GPU[i] + (1-layer_GPU[j]) - 1;
                        model.addConstr(addConstrlhs0,'<',unmap[j][i][0],temp_string0);
                    }
                }
            }
            for (int i = 0; i < layer.size(); i ++)   {
                for (int j = 0; j < i; j ++)    {
                    if (layer_comm[j][i]) {
                        std::string temp_string0 = layer[i];
                        temp_string0 = temp_string0 + "ummap_2";
                        GRBLinExpr addConstrlhs0 = 0;
                        addConstrlhs0 += layer_GPU[i];
                        model.addConstr(addConstrlhs0,'>',unmap[j][i][0],temp_string0);
                    }
            ...
            for (int i = 0; i < layer.size(); i ++)   {
                for (int j = 0; j < i; j ++)    {
                    if (layer_comm[j][i]) {
                        std::string temp_string0 = layer[i];
                        temp_string0 = temp_string0 + "unmap_3";
                        GRBLinExpr addConstrlhs0 = 0;
                ...
                for (int i = 0; i < layer.size(); i ++)   {
                    for (int j = 0; j < i; j ++)    {
                        if (layer_comm[j][i]) {
                            ...
                            model.addConstr(addConstrlhs0,'<',map[j][i][1],temp_string0);
                        }
                    }
                }
                for (int i = 0; i < layer.size(); i ++)   {
                ...
                for (int i = 0; i < layer.size(); i ++)   {
                    for (int j = 0; j < i; j ++)    {
                        if (layer_comm[j][i]) {
                            ...
                            model.addConstr(addConstrlhs0,'<',map[j][i][2],temp_string0);
                        }
                    }
                }
                for (int i = 0; i < layer.size(); i ++)   {
                ...
                for (int i = 0; i < layer.size(); i ++)   {
                    for (int j = 0; j < i; j ++)    {
                        if (layer_comm[j][i]) {
                            ...
                            model.addConstr(addConstrlhs0,'<',map[j][i][3],temp_string0);
                        }
                    }
                }
                for (int i = 0; i < layer.size(); i ++)   {
            ...
            for (int i = 0; i < bottom_layer.size(); i ++)  {
                ...
                for (int l = 0 ; l < bottom_layer[i].size(); l ++)  {
                    for (int j = 0; j < i; j ++)  {
                        if (bottom_layer[i][l] == layer[j]) {
                            addConstrlhs += (map[j][i][0] * (map_alpha * layer_comm[j][i] + map_beta));
                        }
                    }
                }
                model.addConstr(addConstrlhs,'=',map_time[i][0], temp_string);
            }
            if (layer_CPU2.size())  {
                for (int i = 0; i < bottom_layer.size(); i ++)  {
                    ...
                    for (int l = 0 ; l < bottom_layer[i].size(); l ++)  {
                        for (int j = 0; j < i; j ++)  {
                            if (bottom_layer[i][l] == layer[j]) {
                                addConstrlhs += (map[j][i][1] * (map_alpha * layer_comm[j][i] + map_beta));
                            }
                        }
                    }
                    model.addConstr(addConstrlhs,'=',map_time[i][1], temp_string);
                }
            }
            if (layer_CPU3.size())  {
                for (int i = 0; i < bottom_layer.size(); i ++)  {
                    ...
                    for (int l = 0 ; l < bottom_layer[i].size(); l ++)  {
                        for (int j = 0; j < i; j ++)  {
                            if (bottom_layer[i][l] == layer[j]) {
                                addConstrlhs += (map[j][i][2] * (map_alpha * layer_comm[j][i] + map_beta));
                            }
                        }
                    }
                    model.addConstr(addConstrlhs,'=',map_time[i][2], temp_string);
                }
            }
            if (layer_CPU4.size())  {
                for (int i = 0; i < bottom_layer.size(); i ++)  {
                    ...
                    for (int l = 0 ; l < bottom_layer[i].size(); l ++)  {
                        for (int j = 0; j < i; j ++)  {
                            if (bottom_layer[i][l] == layer[j]) {
                                addConstrlhs += (map[j][i][3] * (map_alpha * layer_comm[j][i] + map_beta));
                            }
                        }
                    }
                    model.addConstr(addConstrlhs,'=',map_time[i][3], temp_string);
                }
            }
            ...
            for (int i = 0; i < bottom_layer.size(); i ++)  {
                ...
                for (int j = 0; j < i; j ++)  {
                    for (int l = 0 ; l < bottom_layer[i].size(); l ++)  {
                        if (bottom_layer[i][l] == layer[j]) {
                            addConstrlhs += (unmap[j][i][0] * (unmap_alpha * layer_comm[j][i] + unmap_beta));
                        }
                    }
                }
                model.addConstr(addConstrlhs,'=',unmap_time[i][0], temp_string);
            }
            ...
            for (int i = 1; i < layer.size(); i ++)   {
                ...
                addConstrlhs += map_time[i][0];
                if (layer_CPU2.size())  {
                    addConstrlhs += map_time[i][1];
                }
                if (layer_CPU3.size())  {
                    addConstrlhs += map_time[i][2];
                }
                if (layer_CPU4.size())  {
                    addConstrlhs += map_time[i][3];
                }
                addConstrlhs += unmap_time[i][0];
            ...
            data_for_merging.reserve(layer.size());
            //mapped processor
            for (int i = 0; i < layer.size(); i ++)   {
            ...
            for (int i = 0; i < layer.size(); i ++)   {
                std::cout
                << map_time[i][0].get(GRB_StringAttr_VarName) << ":"
                << abs(map_time[i][0].get(GRB_DoubleAttr_X))<< '\t';
                if (layer_CPU2.size()) {
                    std::cout << map_time[i][1].get(GRB_StringAttr_VarName) << ":"
                    << abs(map_time[i][1].get(GRB_DoubleAttr_X))<< '\t';
                }
                if (layer_CPU3.size()) {
                    std::cout<< map_time[i][2].get(GRB_StringAttr_VarName) << ":"
                    << abs(map_time[i][2].get(GRB_DoubleAttr_X))<< '\t';
                }
                if (layer_CPU4.size()) {
                    std::cout<< map_time[i][3].get(GRB_StringAttr_VarName) << ":"
                    << abs(map_time[i][3].get(GRB_DoubleAttr_X))<< '\t';
                }
                std::cout << unmap_time[i][0].get(GRB_StringAttr_VarName) << ":"
                << abs(unmap_time[i][0].get(GRB_DoubleAttr_X))
                << std::endl;
            }
            ...
            for (int i = 0; i < layer.size(); i ++)   {
                ...
                if ((long)(layer_CPU[i].get(GRB_DoubleAttr_X)+0.5))   {
                    temp_result.mapped = "0";
                    data_for_merging[i]= 0;
                }
                else  {
                    temp_result.mapped = "1";
                ...
                temp_result.comm_time += map_time[i][0].get(GRB_DoubleAttr_X);
                temp_result.comm_time += map_time[i][1].get(GRB_DoubleAttr_X);
                temp_result.comm_time += map_time[i][2].get(GRB_DoubleAttr_X);
                temp_result.comm_time += map_time[i][3].get(GRB_DoubleAttr_X);
                temp_result.comm_time += unmap_time[i][0].get(GRB_DoubleAttr_X);
