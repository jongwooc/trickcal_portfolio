...
int main(int argc, char *argv[])
{
    ...
    std::<std::vector<GRBVar>> end_of_GPU;
...
//        }
        ...
        tempConstrlhs = 0;
        for (int i = 0; i <layer.size(); i ++)  {
            addConstrlhs += (unmap[j] * (unmap_alpha * layer_comm[j][0] + unmap_beta));
        }
        model.addConstr(addConstrlhs,'=',unmap_time, "unmap operation time sum to GPU");
